...
**Latest tested package release:** `22.0.4` for Angular `22.x`
...
| :---: | :---: | :---: | :---: | :--- |
| **22.x** | **Angular 22 tested, Angular 23 install-ready** | **`>=22.0.0 <24.0.0`** | **22.0.4 -> 22.0.2 runtime** | [Angular 22 family docs](https://alexandro.net/docs/angular/multiselect/angular-22/) |
...
```bash
npm install @stackline/angular-multiselect-dropdown@22.0.4 --save-exact
```

Install `22.0.4` for Angular 22.x applications. This patch keeps the tested Angular behavior and opens the peer range through Angular 23.x for launch-day installation. It makes `<angular-multiselect>` the documented standard selector, keeps `<angular2-multiselect>` only as a legacy compatibility alias, includes the accessibility-focused and keyboard/ARIA tested interaction contract, preserves selected objects during async refreshes, and keeps dropdown surfaces opaque in clipped containers.
...
The Angular 22 release was tested in a real Angular `22.0.2` application with `@stackline/angular-multiselect-dropdown@22.0.4`. The docs use the same example pattern from the clean test app, including the accessibility-focused keyboard, focus, and ARIA behavior, responsive dropdown width handling, opaque menu surfaces, and dialog-safe positioning carried forward from the Angular 21.2.x line.
